...
class ProjectGenerator:
    ...
    def _create_env_file(self, config: ProjectConfig) -> None:
        """Create .env file with configured PORT and other environment variables."""
        env_file = self.project_path / ".env"
        env_content = f"""# Environment variables for {config.project_name}
# This file is automatically generated. You can modify it as needed.
...
"""
        with open(env_file, "w", encoding="utf-8") as f:
            f.write(env_content)
        click.echo(click.style("Created .env file with configured PORT", fg="blue"))
